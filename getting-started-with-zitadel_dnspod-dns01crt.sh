...
# 交互式输入DNSPOD API信息
  echo "请输入DNSPod API信息"
  read -p "请输入DNSPod API ID: " DNSPOD_API_ID
  read -p "请输入DNSPod API KEY: " DNSPOD_API_KEY
  DNSPOD_API_TOKEN="$DNSPOD_API_ID,$DNSPOD_API_KEY"

# 交互式输入域名
  if ! check_nb_domain "$NETBIRD_DOMAIN"; then
    echo "请输入您的域名(如netbird.example.com)，或输入'use-ip'使用IP地址"
    NETBIRD_DOMAIN=$(read_nb_domain)
  fi
  ...
  cat <<EOF
{
  ...
  acme_dns dnspod ${DNSPOD_API_TOKEN}  # 全局DNSPod验证
}
...
services:
  # Caddy reverse proxy
  caddy:
    image: nazawsze/caddy-dnspod  #The custom Docker image has DNS01 challenge support
    ...
    environment:
      - DNSPOD_API_TOKEN=${DNSPOD_API_TOKEN} #DNSPODAPI
    logging:
      driver: "json-file"
      options:
        max-size: "500m"
